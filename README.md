...
收集App 是 Web 应用：**服务器跑着，用户打开网址登录即可，无需下载安装任何客户端**。想要 App 般的图标体验，用浏览器的「添加到主屏幕」（PWA）：

1. **拿到网址**——三种来源任选其一：
   - 本机自用：`http://127.0.0.1:8770`
   - 局域网共用：服务端设 `HOST=0.0.0.0` 启动（macOS 可直接双击 `启动收集App.command`），启动日志会打印 `📱 手机同 WiFi 访问：http://192.168.x.x:8770`
   - 公网部署：见下一节，地址就是你的域名
2. **注册 / 登录**——每个用户拥有独立的监控源、资讯流和 AI 配置
3. **装成 App 图标**（可选）：
   - **iPhone / iPad**：Safari 打开网址 → 底部「分享」→「添加到主屏幕」→ 主屏出现 ★ 图标，点开全屏直达
   - **Android**：Chrome 打开网址 → 右上菜单 →「添加到主屏幕」（HTTPS 部署时显示为「安装应用」）
   - **电脑 Chrome/Edge**：地址栏右侧「安装」按钮（需 HTTPS 或 localhost）

> 说明：HTTP 局域网下「添加到主屏幕」即可获得图标和全屏体验；浏览器的完整「安装应用」提示需 HTTPS（公网部署后自动满足）。
...
```bash
# 1. 云服务器上完成上节「Docker 部署」（建议 .env 中 PORT 保持 8770，仅本机监听由反代转发）
# 2. 域名解析：到域名服务商处把 A 记录指向服务器公网 IP（国内服务器需 ICP 备案）
# 3. 安装 Caddy 并写配置 /etc/caddy/Caddyfile：
```
...
```bash
# 4. 重载 Caddy，HTTPS 证书自动签发
sudo systemctl reload caddy
# 5. .env 中设 COOKIE_SECURE=true 后重启容器（HTTPS 下加固会话 cookie）
```

完成后用户访问 `https://你的域名.com` → 登录页，手机「添加到主屏幕」将获得完整 PWA 安装体验。
