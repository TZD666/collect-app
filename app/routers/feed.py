...
@router.post("/feed")
def feed(payload: dict = Body(...), user: dict = Depends(current_user), conn=Depends(get_db)):
    ...
    try:
        ...
        if action == "list":
            ...
            cdb.feed_set_star(conn, fid, star, uid)
            if star > 0 and not f["fulltext"]:
                ...
                try:
                    ...
                    cdb.feed_set_fulltext(conn, fid, True, uid)
                ...
                if f["fulltext"]:
                    ...
                    cdb.feed_set_fulltext(conn, fid, False, uid)
                cutoff = time.strftime(
                    "%Y-%m-%d", time.localtime(time.time() - 7 * 86400))
                removed = False
                if f.get("pub_date") and f["pub_date"] < cutoff:
                    conn.execute("DELETE FROM feed WHERE id=? AND user_id=?", (fid, uid))
                    conn.commit()
                    removed = True
