...
D3 决策：只吃结构化源（RSS / Atom / sitemap）；HTML 列表页发现 = 后续档，本期不做。

职责边界：
  本脚本只把「新条目（标题+链接）」写进 feed 表，不抓全文。
  全文抓取由 server.py 的 /fulltext 端点在用户打星时惰性触发。

用法：
  python3 crawler.py            # 跑所有「到期 + 启用」的源
  python3 crawler.py <source_id> # 只跑指定源（忽略冷却）
  python3 crawler.py --test <url># 试抓某 URL，输出可解析条目数（加源校验用）
"""
...
LOCK_FILE = os.path.join(BASE_DIR, "data", ".crawl.lock")
